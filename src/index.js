  ...
  try {
    ...
    if (ytInitData && ytInitData.length > 1) {
      ...
      const playerResponse =
        ytPlayerData && ytPlayerData.length > 1
          ? ytPlayerData[1].split('</script>')[0].slice(0, -1)
          : null;
...
export const GetPlaylistData = async (playlistId, limit = 0) => {
  ...
  try {
    ...
    const banner =
      metadata?.playlistHeaderBanner?.heroPlaylistThumbnailRenderer?.thumbnail
        ?.thumbnails;

    const channelUrl =
      metadata?.ownerText?.runs[0].navigationEndpoint?.commandMetadata
...
export const GetSuggestData = async (limit = 0) => {
  ...
  try {
    ...
    const richGridRenderer = await page.initData.contents
      .twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content
      .richGridRenderer;

    const options =
      (await richGridRenderer?.header?.feedFilterChipBarRenderer?.contents) ??
      [];
    ...
    (await options.length) &&
      ...
      options.forEach((item) => {
        ...
        const chipToken =
          chipItem?.navigationEndpoint?.continuationCommand?.token ?? null;

        const chipText =
          chipItem.text?.runs.map((x) => x.text).join('') ?? null;
...
export const GetChannelById = async (channelId) => {
  const parseChannelId =
    channelId.indexOf('@') !== -1 ? channelId : `@${channelId}`;
  ...
  try {
    ...
    if (
      ...
      channelHeader.badges.length > 0 &&
      channelHeader.badges[0].metadataBadgeRenderer &&
      ['OFFICIAL_ARTIST_BADGE', 'BADGE_STYLE_TYPE_VERIFIED_ARTIST'].includes(
        channelHeader.badges[0].metadataBadgeRenderer.style
      )
    ) {
      artist = true;
    ...
    const primaryLinks =
      channelHeader?.headerLinks?.channelHeaderLinksRenderer?.primaryLinks;
    const secondaryLinks =
      channelHeader?.headerLinks?.channelHeaderLinksRenderer?.secondaryLinks;
    ...
    const items = tabs
      ...
      .map((x) => x.sectionListRenderer.contents)
      .flat()
      .map((y) => y.itemSectionRenderer.contents)
      .flat();
    ...
    items.map((x, i) => {
      if (x.channelFeaturedContentRenderer) {
        ...
        json.items.map((x) =>
          featuredItems.push(parseVideoRender(x.videoRenderer))
        );
...
export const GetVideoDetails = async (videoId) => {
  ...
  try {
    ...
    let isLive = false;
    if (
      videoInfo?.viewCount?.videoViewCountRenderer?.hasOwnProperty('isLive')
    ) {
      isLive = true;
    }

    const likeCount =
      videoInfo?.videoActions?.menuRenderer?.topLevelButtons[0]
    ...
    const suggestionContext = {
      context: context,
      continuation: suggestionToken,
    };

    const suggestionNextPage = {
      nextPageToken: apiToken,
      nextPageContext: suggestionContext,
    };

    const channelUrl =
      channelInfo.owner.videoOwnerRenderer.navigationEndpoint.commandMetadata
        .webCommandMetadata.url;
    ...
    const channel = {
      ...
      subscriber:
        channelInfo.owner.videoOwnerRenderer.subscriberCountText.simpleText,
    ...
    const res = {
      ...
      likes: likeCount,
      publishedAt: isLive
        ? videoInfo?.dateText?.simpleText
        : videoInfo?.relativeDateText?.simpleText,
      description:
        player.shortDescription ?? channelInfo.attributedDescription.content,
      channel,
      player,
...
export const getComments = async (nextPage) => {
  const endpoint =
    await `${youtubeEndpoint}/youtubei/v1/next?key=${nextPage.nextPageToken}`;
  const items = [];
  ...
  try {
    ...
    const commentHeader =
      response[0]?.reloadContinuationItemsCommand?.continuationItems[0]
        ?.commentsHeaderRenderer;

    const commentCounts = commentHeader?.countText?.runs
      ?.map((x) => x.text)
      .join('');
    ...
    try {
      commentKeys =
        response[1]?.reloadContinuationItemsCommand.continuationItems
          .filter((x) => x.commentThreadRenderer)
          .map((x) => {
            ...
            if (reply) {
              token =
                reply.commentRepliesRenderer.contents[0]
    ...
    const getItemList =
      page.data?.frameworkUpdates?.entityBatchUpdate?.mutations || null;
    ...
    for (const conitem of itemList) {
      ...
      if (commentThread) {
        ...
        const channelUrl =
          commentThread.author.channelCommand.innertubeCommand.commandMetadata.webCommandMetadata.url?.replace(
            '/',
            ''
          );
  ...
  try {
    ...
    for (const conitem of itemList.continuationItems) {
      ...
      if (commentThreadRenderer) {
        ...
        const repliesToken =
          reply?.contents[0]?.continuationItemRenderer?.continuationEndpoint
            ?.continuationCommand?.token;

        let artist = false;
        if (
          comment.authorCommentBadge &&
          ...
          [
            ...
            comment.authorCommentBadge.authorCommentBadgeRenderer.icon.iconType
          )
        ) {
          artist = true;
        }
        ...
        if (
          ...
          comment.authorCommentBadge.authorCommentBadgeRenderer &&
          comment.authorCommentBadge.authorCommentBadgeRenderer.icon &&
          ['CHECK', 'CHECK_CIRCLE_THICK'].includes(
            comment.authorCommentBadge.authorCommentBadgeRenderer.icon.iconType
          )
        ) {
          verified = true;
        ...
        const commentItem = {
          ...
          publishedAt: comment.publishedTimeText?.runs
            ?.map((x) => x.text)
            .join(''),
    ...
    try {
      response[1]?.reloadContinuationItemsCommand.continuationItems
        ...
        .map((x) => {
          ...
          if (reply) {
            token =
              reply.commentRepliesRenderer.contents[0].continuationItemRenderer
                .continuationEndpoint.continuationCommand.token;
          }
...
export const VideoRender = (json) => {
  try {
    if (json && (json.videoRenderer || json.playlistVideoRenderer)) {
      ...
      const views = videoRenderer.viewCountText
...
export const compactVideoRenderer = (json) => {
  ...
  if (
    ...
    badges = compactVideoRendererJson.badges.map(
      (x) => x.metadataBadgeRenderer.label
    );
  ...
  const viewsCount = isLive
    ? compactVideoRendererJson.shortViewCountText?.runs
        ?.map((x) => x.text)
        .join('')
    : compactVideoRendererJson.shortViewCountText?.simpleText;
...
export async function getTrending() {
  ...
  try {
    ...
    const results = await page.initData.contents.twoColumnBrowseResultsRenderer
      .tabs[0].tabRenderer.content;
    const items = results.sectionListRenderer.contents
      .filter((x) => x.itemSectionRenderer)
      .map((z) => z.itemSectionRenderer.contents)
      .flat();
    ...
    items.length &&
      items.map((x) => {
        if (x.shelfRenderer) {
          ...
          shortsResult.map(({ reelItemRenderer: json }) =>
            shortsList.push({
              id: json.videoId,
              type: 'reel',
...
export async function getFeed(name) {
  ...
  const contentHeader =
    (await page.initData?.header?.c4TabbedHeaderRenderer) ||
    page.initData?.header?.carouselHeaderRenderer;
  ...
  if (contentHeader?.contents) {
      ...
      if (x.carouselItemRenderer) {
        ...
        const promo = carouselItems.map((x) => {
          ...
          return {
            ...
            url: promo?.navigationEndpoint?.commandMetadata?.webCommandMetadata.url?.split(
              'v='
            )[1],
          };
        });
        ...
        headerItems.avatar =
          topicChannelDetailsRenderer?.avatar?.thumbnails?.pop();
        headerItems.subscriber =
          topicChannelDetailsRenderer?.subtitle?.simpleText;
      }
    ...
    headerItems = {
      title: contentHeader?.title,
      subtitle: contentHeader?.subscriberCountText?.runs
        ?.map((x) => x.text)
        ?.join(''),
      avatar: contentHeader?.avatar?.thumbnails[0],
    };
  ...
  if (results.richGridRenderer) {
    ...
    richGridRenderer.map((x) => {
      if (x.richSectionRenderer) {
        ...
        const title =
          richContent.richShelfRenderer?.title?.simpleText ||
          richContent.richShelfRenderer?.title?.runs
            ?.map((x) => x.text)
            .join('');

        const subtitle = richContent.richShelfRenderer?.subtitle?.runs
          ?.map((x) => x.text)
          .join('');
        const richItemRenderer = richContent.richShelfRenderer.contents.map(
          (x) => x.richItemRenderer
        );
    ...
    const itemSectionRenderer =
      results.sectionListRenderer.contents.map((x) => x?.itemSectionRenderer) ??
      [];

    const itemSectionContent = itemSectionRenderer
      .filter((x) => x)
      ?.map((x) => x.contents)
      ?.flat();
    ...
    itemSectionContent.map((x) => {
      if (x.horizontalCardListRenderer) {
        ...
        const cardHeader =
          horizontalCardListRenderer.header?.richListHeaderRenderer;

        const title =
          cardHeader?.title?.simpleText ||
          cardHeader?.title?.runs?.map((x) => x.text).join('');

        const subtitle =
          cardHeader?.subtitle?.simpleText ||
          cardHeader?.subtitle?.runs?.map((x) => x.text).join('');
        ...
        const title =
          ...
          shelfRenderer?.title?.runs?.map((x) => x.text).join('');

        const subtitle =
          shelfRenderer?.subtitle?.simpleText ||
          shelfRenderer?.subtitle?.runs?.map((x) => x.text).join('');
        ...
        const items = shelfRenderer.items.map((x) =>
          shortVideoParser(x.reelItemRenderer)
        );
...
export const GetHomeFeed = async () => {
  ...
  items.map((x) => {
    if (x.continuationItemRenderer) {
      contToken =
        x.continuationItemRenderer.continuationEndpoint.continuationCommand
          .token;
    } else {
  ...
  try {
    ...
    formats.length &&
      formats.map((x) => {
        ...
        const parsedUrl =
          x.url ?? decodeURI(x?.signatureCipher?.split('&url=')[1] ?? '');

        player.media.push({
          url: parsedUrl,
          hls: videoDetails.isLive
...
export function parseVideoRender(response) {
  ...
  try {
    ...
    const channelUrl =
      response.ownerText.runs[0].navigationEndpoint.commandMetadata
        .webCommandMetadata.url;
    const channelAvatar =
      response.channelThumbnailSupportedRenderers
    ...
    const descriptionSnippet = hasDescription
      ? response?.detailedMetadataSnippets[0].snippetText?.runs
          ?.map((x) => x.text)
          .join('')
      : false;

    const descriptionTwoSnippet = hasDescriptionSnippet
      ? response.descriptionSnippet?.runs?.map((x) => x.text).join('')
      : false;

    const viewsCount = isLive
      ? response.shortViewCountText?.runs?.map((x) => x.text).join('')
      : response.shortViewCountText?.simpleText;
    ...
    const result = {
      ...
      views: viewsCount,
      publishedAt: isLive
        ? response?.dateText?.simpleText
        : response?.publishedTimeText?.simpleText,
...
function parseChannelRender(response) {
  try {
    const channelUrl =
      response.shortBylineText.runs[0].navigationEndpoint.commandMetadata
        .webCommandMetadata.url;
